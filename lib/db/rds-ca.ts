// Amazon RDS certificate authority for ap-southeast-1.
//
// GENERATED by aws/scripts/fetch-rds-ca.sh. Don't hand-edit, re-run the script.
// Source: https://truststore.pki.rds.amazonaws.com/ap-southeast-1/ap-southeast-1-bundle.pem
//
// A TS module rather than a file on disk because it has to reach three bundlers intact (the
// Next standalone build, esbuild for the workers, tsx for the harnesses) and a string constant
// is just an import, so it can't be silently left out of one. Node's trust store doesn't carry
// the RDS roots, so without this `rejectUnauthorized: true` fails against every endpoint - and
// the tempting fix is turning it off, which discards the only defence the open SG has.
// Valid to 2121, so re-run the script only if AWS publishes a new regional root.

/** The full chain, PEM. Pass as `ssl.ca` to node-postgres. */
export const RDS_CA_BUNDLE = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
