...
import { getMeeting, listMeetings, listProjects } from "@/lib/store";
import { Card, CardHeader, CardBody } from "@/components/ui/card";
import { Pill } from "@/components/ui/badge";
import { ReviewPanel } from "@/components/meetings/review-panel";
...
export default async function ReviewPage({
  ...
  const [meeting, projects, meetings] = await Promise.all([
    getMeeting(id),
    listProjects(),
    listMeetings(),
  ]);
  ...
  return (
    <main className="mx-auto max-w-[820px] px-8 py-8">
      ...
      {(meeting.daily_objective || meeting.summary) && (
        <Card className="mt-5">
      ...
      {!isPlan && meeting.decisions.length > 0 && (
        <Card className="mt-5">
          <CardHeader
            title="Decisions"
            icon={<GitBranch className="size-4" />}
            action={<Pill>{meeting.decisions.length}</Pill>}
          />
      ...
      {!isPlan && meeting.open_questions.length > 0 && (
        <Card className="mt-5">
          <CardHeader
            title="Open questions"
            icon={<HelpCircle className="size-4" />}
            action={<Pill>{meeting.open_questions.length}</Pill>}
          />
          <ul className="divide-y divide-[var(--color-border)]">
