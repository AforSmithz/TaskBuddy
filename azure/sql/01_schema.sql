-- TaskBuddy schema for Azure Database for PostgreSQL (Flexible Server).
--
-- This is the Supabase schema translated off Supabase's managed auth. Three
-- things change; everything else is byte-for-byte the same table shape.
--
--   1. `auth.users(id)`  ->  `users(id)`, a table we own (see 00 below).
--   2. `auth.uid()`      ->  `app.uid()`, which reads a per-transaction GUC
--                            the app sets from the session cookie.
--   3. `force row level security` on every table except `users`, so RLS still
--      applies even when the connection happens to own the table. `users` is
--      the one carve-out, because login has to read it before a session
--      exists — see the note above the FORCE block and 03_auth.sql.
--
-- Run order:  01_schema.sql  ->  02_grants.sql  ->  03_auth.sql
-- Connect as the server admin created with the Flexible Server.
--
...
-- on the azure.extensions allowlist, since nothing below depends on it.
do $$
begin
...
-- and citext may not be on the azure.extensions allowlist. The app normalises
